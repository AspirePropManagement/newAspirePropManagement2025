-- =====================================================
-- FIX PROPERTY TABLES WITH ENHANCED IMAGE SUPPORT
-- Based on existing table structure and ALTER_QUERIES_FOR_PROPERTY_IMAGES.sql
-- =====================================================

-- =====================================================
-- 1. ALTER NEW_PROJECTS TABLE
-- =====================================================

-- Add specific image category columns to new_projects
...
CREATE INDEX IF NOT EXISTS idx_new_projects_legal_docs ON new_projects USING GIN(legal_docs);
CREATE INDEX IF NOT EXISTS idx_new_projects_virtual_content ON new_projects USING GIN(virtual_content);

-- =====================================================
-- 2. ALTER RENTAL_PROPERTIES TABLE
-- =====================================================

-- Add specific image category columns to rental_properties
...
CREATE INDEX IF NOT EXISTS idx_rental_properties_legal_docs ON rental_properties USING GIN(legal_docs);
CREATE INDEX IF NOT EXISTS idx_rental_properties_virtual_content ON rental_properties USING GIN(virtual_content);

-- =====================================================
-- 3. ALTER RESALE_PROPERTIES TABLE
-- =====================================================

-- Add specific image category columns to resale_properties
...
CREATE INDEX IF NOT EXISTS idx_resale_properties_legal_docs ON resale_properties USING GIN(legal_docs);
CREATE INDEX IF NOT EXISTS idx_resale_properties_virtual_content ON resale_properties USING GIN(virtual_content);

-- =====================================================
-- 4. CREATE IMAGE CATEGORIES REFERENCE TABLE
-- =====================================================
...
-- =====================================================
-- 5. CREATE PROPERTY IMAGES DETAILED TRACKING TABLE
-- =====================================================

CREATE TABLE IF NOT EXISTS property_image_details (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  property_id UUID NOT NULL,
  property_type TEXT NOT NULL CHECK (property_type IN ('resale', 'rental', 'new_project')),
...
-- =====================================================
-- 6. SAMPLE DATA INSERTION QUERIES
-- =====================================================
...
-- Sample rental property with comprehensive images
INSERT INTO rental_properties (
  owner_name, owner_email, owner_contact_no, property_type, bhk_type, 
  location, rent_amount, furnishing_type, allowed_for_family, allowed_for_bachelor, allowed_for_anyone,
  general_photos, floor_plans, legal_docs, virtual_content, created_by
) VALUES (
  'Priya Sharma', 'priya.sharma@email.com', '+919876543211', 'apartment', '2_bhk',
  'Koregaon Park, Pune', 25000, 'semi_furnished', true, true, false,
  '{
    "exterior": ["https://example.com/rental_exterior1.jpg"],
    "interior": ["https://example.com/rental_interior1.jpg"],
    "bedrooms": ["https://example.com/rental_bedroom1.jpg"],
    "kitchen": ["https://example.com/rental_kitchen1.jpg"],
    "bathrooms": ["https://example.com/rental_bathroom1.jpg"],
    "living_dining": ["https://example.com/rental_living1.jpg"]
  }',
  '{
    "floor_plan": ["https://example.com/rental_floorplan1.jpg"]
  }',
  '{
    "legal_documents": ["https://example.com/rental_legal1.pdf"]
  }',
  '{
    "virtual_tour": ["https://example.com/rental_virtual1.mp4"]
  }',
  (SELECT id FROM users WHERE role = 'AGENT' LIMIT 1)
);
...
WHERE table_name IN ('new_projects', 'rental_properties', 'resale_properties')
AND column_name IN ('general_photos', 'floor_plans', 'project_images', 'legal_docs', 'virtual_content')
ORDER BY table_name, column_name;

-- Check if indexes were created
SELECT indexname, tablename 
FROM pg_indexes 
WHERE tablename IN ('new_projects', 'rental_properties', 'resale_properties')
AND (indexname LIKE '%general_photos%' OR indexname LIKE '%floor_plans%' OR indexname LIKE '%project_images%' OR indexname LIKE '%legal_docs%' OR indexname LIKE '%virtual_content%')
ORDER BY tablename, indexname;

-- Check sample data for new_projects
SELECT 
  'new_projects' as table_name,
  project_name, 
  crafted_by,
  jsonb_pretty(general_photos) as general_photos_json
FROM new_projects 
WHERE general_photos IS NOT NULL 
LIMIT 1;

-- Check sample data for resale_properties
SELECT 
  'resale_properties' as table_name,
  seller_name as name, 
  property_type,
  jsonb_pretty(general_photos) as general_photos_json
...
-- Check sample data for rental_properties
SELECT 
  'rental_properties' as table_name,
  owner_name as name, 
  property_type,
  jsonb_pretty(general_photos) as general_photos_json
