...
/**
 * Admin Builders page - displays all builder users with admin management capabilities
 * Includes tabs for active builders and pending approvals with click-to-view details
 */
export default function AdminBuildersPage() {
  const { users, loading, error, updateUser, deleteUser, toggleUserStatus, refreshUsers } = useUserManagement('BUILDER');
  ...
  const fetchPendingUsers = async () => {
    try {
      ...
      const pending = await getPendingUsersByRole('BUILDER');
      setPendingUsers(pending);
    } catch (err) {
      console.error('Error fetching pending builders:', err);
    } finally {
  ...
  const handleApprove = async (userId: string) => {
    try {
      ...
      await refreshUsers();
      showSuccess('Builder approved successfully!');
    } catch (error) {
  ...
  const handleRejectConfirm = async (reason: string) => {
    ...
    try {
      ...
      await fetchPendingUsers();
      showSuccess('Builder rejected successfully.');
    } catch (error) {
      ...
      <DashboardLayout>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="mb-4 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Builder Management</h1>
            <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">Manage all builder accounts in the system</p>
          </div>
          <InlinePreloader text="Fetching builders..." />
        </div>
      </DashboardLayout>
    );
  }
      ...
      <DashboardLayout>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="text-center py-12">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Error Loading Builders</h1>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="border-b border-gray-200 mb-4 sm:mb-6">
          <nav className="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto">
            ...
            >
              Active Builders ({filteredActiveUsers.length})
            </button>
          </nav>
        </div>
        ...
        {activeTab === 'pending' ? (
          <div className="bg-white shadow-md rounded-lg overflow-hidden">
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
              <h2 className="text-base sm:text-lg font-medium text-gray-900">Builders Waiting for Approval</h2>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Click on any row to view details, or use quick approve/reject actions</p>
            </div>
            {pendingLoading ? (
              <div className="p-6">
                <InlinePreloader text="Loading pending builders..." />
              </div>
            ) : pendingUsers.length === 0 ? (
              <div className="text-center py-12">
                <div className="text-gray-400 text-4xl sm:text-6xl mb-4">✅</div>
                <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No Pending Approvals</h3>
                <p className="text-sm sm:text-base text-gray-600">All builder registrations have been reviewed.</p>
              </div>
            ) : (
            ...
            {filteredActiveUsers.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">
                  {users.length === 0 ? 'No Builders Found' : 'No Active Builders Match Your Search'}
                </h2>
                <p className="text-sm sm:text-base text-gray-600">
                  {users.length === 0 
                    ? 'There are currently no approved builder accounts in the system.'
